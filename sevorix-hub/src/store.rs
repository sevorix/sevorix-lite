...
impl AzureBlobStore {
    ...
    fn blob_url(&self, key: &str) -> String {
        ...
        format!(
            ...
            self.account, self.container, encoded_key
        )
    }
...
mod tests {
    ...
    fn test_azure_blob_url() {
        ...
        // Slashes and dots are encoded by NON_ALPHANUMERIC
        assert_eq!(
            url,
            "https://myaccount.blob.core.windows.net/artifacts/artifacts%2Ftest%2Ejson"
        );
    }
    ...
    fn test_azure_blob_url_contains_account_and_container() {
        ...
        // Slashes are percent-encoded; the encoded form of "some/key.json" is present
        assert!(url.contains("some%2Fkey%2Ejson"));
        assert!(url.starts_with("https://"));
        assert!(url.contains(".blob.core.windows.net"));
    }

    #[test]
    fn test_azure_blob_url_encodes_special_characters() {
        let store = AzureBlobStore::new("acct", "cont");
        // Keys with /, ?, #, % must all be encoded
        let url = store.blob_url("path/to/artifact?v=1#frag%done");
        // The encoded key portion must not contain raw ?, #, or unencoded %
        assert!(!url.contains('?'), "unencoded '?' found in URL");
        assert!(!url.contains('#'), "unencoded '#' found in URL");
        // Slashes in the key become %2F
        assert!(url.contains("%2F"), "expected %2F for encoded slash");
        // The original % in the key becomes %25
        assert!(url.contains("%25"), "expected %25 for encoded percent sign");
    }
