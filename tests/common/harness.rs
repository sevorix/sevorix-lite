...
impl TestHarness {
    ...
    /// Spin up a server with pre-registered hooks.
    ///
    /// Because `hook_registry` in `AppState` is `Arc<HookRegistry>` (not
    /// `Arc<RwLock<HookRegistry>>`), hooks must be registered before the
    /// registry is wrapped in an `Arc`.  Use this constructor when tests need
    /// to inject hooks; use `new()` / `with_role()` for hook-free tests.
    pub async fn with_hooks(hooks: Vec<Box<dyn sevorix_watchtower::hooks::Hook>>) -> Self {
        Self::with_role_and_hooks(None, hooks).await
    }

    /// Spin up a server with an optional default role and pre-registered hooks.
    pub async fn with_role_and_hooks(
        default_role: Option<String>,
        hooks: Vec<Box<dyn sevorix_watchtower::hooks::Hook>>,
    ) -> Self {
        let dir = TempDir::new().unwrap();
        let config_dir = dir.path().to_path_buf();
        ...
        let mut registry = sevorix_watchtower::hooks::HookRegistry::new();
        for hook in hooks {
        ...
        let state = Arc::new(AppState {
            ...
            http_client: reqwest::Client::builder()
                ...
                .unwrap_or_default(),
            tls_context: None,
            hook_registry: std::sync::Arc::new(registry),
        });
