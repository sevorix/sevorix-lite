//! OpenAI Codex integration for Sevorix.
//!
//! This integration configures Codex CLI to use sevsh as its shell,
...
/// Codex integration.
///
/// Modifies `~/.codex/config.json` to set the SHELL environment variable
/// to sevsh, ensuring all shell commands from Codex are validated
/// through Sevorix.
pub struct CodexIntegration {
    /// Path to the sevsh binary.
    sevsh_path: PathBuf,
    /// Path to Codex config (JSON format).
    config_path: PathBuf,
...
impl CodexIntegration {
    /// Create a new Codex integration instance.
...
impl Integration for CodexIntegration {
    ...
    fn description(&self) -> &str {
        "Configures OpenAI Codex CLI to use sevsh as its shell for command validation"
    }
    ...
    fn is_installed(&self) -> bool {
        ...
        // Check if config contains our SHELL configuration
    ...
    fn install(&self) -> Result<InstallResult> {
        ...
        config_changes.push(format!(
            "Set SHELL to {} in ~/.codex/config.json",
            self.sevsh_path_string()
        ));

        let message = if had_existing_shell {
            "Codex integration installed (replaced existing SHELL setting)."
        } else {
            "Codex integration installed. All shell commands from Codex will now be validated through Sevorix."
        };
    ...
    fn uninstall(&self) -> Result<()> {
        ...
        println!("Codex integration uninstalled.");
        println!("The SHELL setting has been removed from ~/.codex/config.json");
...
mod tests {
    ...
    fn create_test_integration(temp_dir: &TempDir) -> CodexIntegration {
        let sevsh_path = temp_dir.path().join(".local/bin/sevsh");
        let config_path = temp_dir.path().join(".codex/config.json");
        ...
        CodexIntegration::new_for_test(sevsh_path, config_path, state_dir)
    }
    ...
    fn test_integration_name() {
        let integration = CodexIntegration::new().unwrap();
    ...
    fn test_description() {
        let integration = CodexIntegration::new().unwrap();
        assert!(integration.description().contains("Codex"));
    ...
    fn test_is_installed_with_different_shell() {
        ...
        let config = r#"{"env": {"SHELL": "/bin/zsh"}}"#;
        std::fs::write(&integration.config_path, config).unwrap();
    ...
    fn test_is_sevsh_installed_false() {
        ...
        let integration = CodexIntegration::new_for_test(sevsh_path, config_path, state_dir);
        assert!(!integration.is_sevsh_installed());
    }
    ...
    fn test_install_success() {
        ...
        assert!(result.message.contains("Codex"));
    ...
    fn test_install_replaces_existing_shell() {
        ...
        std::fs::write(&integration.config_path, r#"{"env": {"SHELL": "/bin/fish"}}"#).unwrap();
    ...
    fn test_uninstall_removes_shell_setting() {
        ...
        std::fs::write(&integration.config_path, r#"{"env": {"SHELL": "~/.local/bin/sevsh", "PATH": "/usr/bin"}}"#).unwrap();

        integration.uninstall().unwrap();

        let config = integration.read_config().unwrap();
        assert!(!config["env"].get("SHELL").is_some());
        assert_eq!(config["env"]["PATH"], "/usr/bin");
    }
    ...
    fn test_install_preserves_other_config() {
        ...
        std::fs::write(&integration.config_path, r#"{"model": "gpt-4", "max_tokens": 1000}"#).unwrap();
